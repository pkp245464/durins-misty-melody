...
@ControllerAdvice
public class GlobalDurinAnalyticsServiceExceptionHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(GlobalDurinAnalyticsServiceExceptionHandler.class);

    @ExceptionHandler(RuntimeException.class)
    public ResponseEntity<ErrorInfo> handleRuntimeExceptionForOthers(HttpServletRequest req, RuntimeException ex) {
        LOGGER.error("GlobalDurinAnalyticsServiceException occurred: {}", ex.getMessage());
    ...
    @ExceptionHandler(GlobalDurinAnalyticsServiceException.class)
    public ResponseEntity<ErrorInfo> handlerGlobalDurinUserServiceException(GlobalDurinAnalyticsServiceException globalDurinAnalyticsServiceException) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ErrorInfo(
                null,
                "GlobalDurinAnalyticsServiceException",
                globalDurinAnalyticsServiceException.getMessage(),
                new Date()
        ));
    }
